...
@rooms_router.get("/{room_id}", dependencies=[Depends(getUser), Depends(isRoomMember)])
async def getRoom(room: RoomDep, service: ServiceDep):
    return await service.getRoom(room)

@rooms_router.put("/{room_id}", dependencies=[Depends(getUser), Depends(isRoomOwner)])
async def editRoom(data: RoomEdit, room: RoomDep, service: ServiceDep):
    return await service.editRoom(data, room)

@rooms_router.delete("/{room_id}", dependencies=[Depends(getUser), Depends(isRoomOwner)])
